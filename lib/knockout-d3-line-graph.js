...
(function (root, factory) {
    ...
    (this, function (ko, d3) {
        ...
        function getPaintingMethods(data, element, options) {
            var yDomain = d3.extent(data, options.y);
            if (options.showBaseLine) {
                var baseLineDomain = d3.extent(data, options.baseLine);
                yDomain = [
                    Math.min(yDomain[0], baseLineDomain[0]),
                    Math.max(yDomain[1], baseLineDomain[1])
                ];
            }
