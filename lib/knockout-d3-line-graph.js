    ...
    (this, function (ko, d3) {
        ...
        function getPaintingMethods(data, element, options) {
            ...
            if (yDomain[0] === undefined) {
                yDomain[0] = options.showBaseLine
                    ? Math.min(dataDomain[0], baseLineDomain[0])
                    : dataDomain[0];
            }

            if(yDomain[1] === undefined){
                yDomain[1] = options.showBaseLine
                    ? Math.max(dataDomain[1], baseLineDomain[1])
                    : dataDomain[1];
            }
            ...
            return {
                ...
                baseLineArea: d3.svg.area().interpolate('basis')
                    ...
                    .y1(height),
                dataKeys: data.map(function(item) { return options.grouping(item); }).unique()
            };
        };
        ...
        ko.bindingHandlers.d3LineGraph = {
            init: function (element, valueAccessor, allBindings) {
                ...
                shapes.dataKeys.forEach(function(groupingKey) {
                    plot.append('path').attr('class', 'path').attr('id', 'line-' + groupingKey).attr('d', shapes.line(data.filter(function(x) { 
                        return options.grouping(x) === groupingKey;
                    })));
                });

                if(shapes.dataKeys.length === 1) {
            ...
            update: function (element, valueAccessor, allBindings) {
                ...
                shapes.dataKeys.forEach(function(groupingKey) {
                    svg.select('path#line-' + groupingKey)
                    .interrupt()
                    .transition()
                    .ease('linear')
                    .duration(250)
                    .attr('d', shapes.line(data.filter(function(x) { 
                        return options.grouping(x) === groupingKey;
                    })));
                });
